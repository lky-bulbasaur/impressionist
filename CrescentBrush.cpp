...
CrescentBrush::CrescentBrush(ImpressionistDoc* pDoc, char* name) : ImpBrush(pDoc, name) {
...
void CrescentBrush::BrushBegin(const Point source, const Point target) {
...
void CrescentBrush::BrushMove(const Point source, const Point target) {
	ImpressionistDoc* pDoc = GetDocument();
	ImpressionistUI* dlg = pDoc->m_pUI;

	if (pDoc == NULL) {
		printf("CrescentBrush::BrushMove  document is NULL\n");
		return;
	}
	...
	double xTrans = radius * sinValues[(angle + 270) % NUM_SEGMENTS];
	double yTrans = radius * cosValues[(angle + 270) % NUM_SEGMENTS];
...
void CrescentBrush::BrushEnd(const Point source, const Point target) {
	// do nothing so far
}